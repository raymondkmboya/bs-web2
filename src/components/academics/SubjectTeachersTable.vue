...
function getSeverity(teacher) {
    // Custom severity based on class load
    if (teacher.totalStudents > 140) return 'warning';
    if (teacher.totalStudents < 80) return 'info';
    return 'success';
}
...
<template>
    ...
    >
        ...
        <Column field="avgStudentsPerClass" header="Avg/Class" sortable>
            <template #body="{ data }">
                <div class="text-center">
                    <span class="font-bold text-blue-600">
                        {{ Math.round(data.totalStudents / data.totalClasses) }}
                    </span>
                </div>
            </template>
        </Column>
