...
const statusOptions = [
    ...
    { label: 'Stop Follow-up', value: 'Stop Follow Up' }
];
...
function resetForm() {
    ...
    // Alert to verify student ID is captured
    // if (props.student?.student_id) {
    //     alert('Student ID captured by form: ' + props.student.student_id);
    //     alert(props.student.student_id)
    //     console.log('Student ID:', props.student.student_id);
    //     console.log('Form data student_id:', formData.value.student_id);
    // } else {
...
async function handleSubmit() {
    ...
    try {
        ...
        if (error.response?.status === 422 && error.response?.data?.errors) {
            ...
            // Handle other errors
            errors.value = {
                general: [error.response?.data?.message || 'Failed to create follow-up. Please try again.']
            };
        }
    } finally {
        loading.value = false;
